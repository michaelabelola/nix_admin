...
import {
    ...
    StepSection,
    StepSelect
} from "#/modules/customer/create/customer-create.fields.tsx"
    ...
    return (
        <CustomerRegisterStepLayout>
            <RegisterStepInsight icon={UserRound} title="Build the profile at your pace">
                Profile fields are optional during registration. Add what is available now and complete the rest after
            ...
            >
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                    <StepInput label="First name" value={personal.firstName}
                               onChange={(value) => updatePersonalDetail({firstName: value})}/>
                    <StepInput label="Middle name" value={personal.middleName}
                               onChange={(value) => updatePersonalDetail({middleName: value})}/>
                    <StepInput label="Last name" value={personal.lastName}
                               onChange={(value) => updatePersonalDetail({lastName: value})}/>
                    <StepInput
                        label="Display name"
                    ...
                    <StepSelect label="Gender" value={personal.gender} placeholder="Select gender"
                                options={GENDER_OPTIONS} onChange={(value) => updatePersonalDetail({gender: value})}/>
                    ...
                    <StepInput label="Profession" value={personal.profession}
                               onChange={(value) => updatePersonalDetail({profession: value})}/>
                </div>
            </StepSection>
        </CustomerRegisterStepLayout>
    )
