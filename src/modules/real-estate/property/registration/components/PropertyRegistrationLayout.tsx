...
export function PropertyRegistrationLayout({
  ...
  return (
    <div className="min-h-full w-full overflow-hidden rounded-2xl border bg-background shadow-xl flex flex-col-reverse lg:grid lg:grid-cols-[340px_1fr]">
        ...
        <div className="relative z-10 grid h-fit gap-8">
          <div className="grid gap-4">
            ...
            <div>
              ...
              <p className="mt-2 text-sm leading-6 text-background/70">
                Create the property, add presentation details, attach storage, set location, and finish onboarding in a route-backed flow.
              </p>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-background/15 bg-background/10 p-4">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {property?.type ? <Badge className="bg-background/15 text-background hover:bg-background/20">{property.type}</Badge> : null}
              {property?.lifecycleStage ? (
                <Badge className="bg-background/15 text-background hover:bg-background/20">{property.lifecycleStage}</Badge>
              ) : null}
              {property?.storageID ? (
                <Badge className="bg-background/15 text-background hover:bg-background/20">Storage active</Badge>
              ) : null}
            </div>
          </div>
          ...
          <div className="mt-auto rounded-xl border border-background/15 bg-background/10 p-4">
            ...
            <p className="mt-2 text-xs leading-5 text-background/70">
              Required setup happens first; optional media, storage, and tags can be skipped and refined later.
            </p>
          </div>
        </div>
      </aside>
